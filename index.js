...
const {
  InitializeLogging,
} = require("./resources/functions/init/Init-Logging");
InitializeLogging();
...
function CreateWindow() {
  ...
  const {
    InstanceHandler,
  } = require("./resources/functions/handler/InstanceHandler");
  ...
  const {
    SettingsHandler,
  } = require("./resources/functions/handler/SettingsHandler");
  SettingsHandler(); // Handles updates to settings

  const {
    WindowStateHandler,
  } = require("./resources/functions/handler/WindowStateHandler");
...
const {SetThumbarButtons} = require('./resources/functions/win/SetThumbarButtons')
  ...
  if (app.preferences.value("general.incognitoMode").includes(true)) {
    new Notification({
      ...
      body: `Incognito Mode enabled. Song Info Receivers will be disabled.`,
    }).show();
    console.log(
      "[Incognito] Incognito Mode enabled. Turning off Discord RPC, LastFM, MPRIS."
    );
  }
}
