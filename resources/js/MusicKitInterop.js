...
const MusicKitInterop = {
  init: function () {
    MusicKit.getInstance().addEventListener(
      ...
      () => {
        if (
          MusicKitInterop.getAttributes().title !== "No Title Found" &&
          MusicKitInterop.getAttributes().playParams.id !== "no-id-found" &&
          ...
          if (
            ...
            console.log(
              `[MusicKitInterop] Sending playbackStateDidChange (${
                MusicKit.getInstance().isPlaying
                  ? "Is Playing"
                  : "Is not Playing"
              })`
            );
    ...
    MusicKit.getInstance().addEventListener(
      ...
      () => {
        if (
          ...
          if (
            CachedAttributes.playParams.id !==
            MusicKitInterop.getAttributes().playParams.id
          ) {
