...
try {
	if (
		document.getElementsByClassName('web-chrome').length > 0 &&
		...
		const redStoplightStyle =
			'height: 11px; width: 11px; background-color: rgb(255, 92, 92); border-radius: 50%; display: inline-block; float: right; top: 0px; margin: 10px 10px 10px 4px; color: rgb(130, 0, 5); fill: rgb(130, 0, 5); -webkit-app-region: no-drag; ';
		const redStoplightOnClick = "ipcRenderer.send('close');";

		/* Green - Maximize */
		const greenStoplightStyle =
			'height: 11px; width: 11px; background-color: rgb(0, 202, 86); border-radius: 50%; display: inline-block; float: right; top: 0px; margin: 10px 4px 10px; color: rgb(130, 0, 5); fill: rgb(130, 0, 5); -webkit-app-region: no-drag;';
		const greenStoplightOnClick = "ipcRenderer.send('maximize');";

		/* Yellow - Minimize */
		const yellowStoplightStyle =
			'height: 11px; width: 11px; background-color: rgb(255, 189, 76); border-radius: 50%; display: inline-block; float: right; top: 0px; margin: 10px 4px; color: rgb(130, 0, 5); fill: rgb(130, 0, 5); -webkit-app-region: no-drag;';
		...
		document.head.insertAdjacentHTML(
			'beforeend',
			'<style>.web-chrome { top: 25px; }</style>'
		);

		/* Add the Stoplights! */
		document.getElementById('web-main').insertAdjacentHTML(
			'afterbegin',
			`
	...
	if (
		document.getElementsByClassName(
			'search-scope-bar search__scope-bar search-scope-bar--desktop-search'
		).length > 0
	) {
		document.getElementsByClassName(
			'search-scope-bar search__scope-bar search-scope-bar--desktop-search'
	...
	console.error(
		'[JS] Error while trying to apply emulateMacOS_rightAlign.js',
		e
	);
}
