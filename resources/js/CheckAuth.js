try {
  if (MusicKit.getInstance().isAuthorized) {
    ...
    if (preferences.general.startupPage.includes("library/"))
      urlToLoad = `${
        preferences.advanced.useBetaSite.includes(true)
          ? `https://beta.music.apple.com`
          : `https://music.apple.com`
      }/${preferences.general.startupPage}?${urlToLoad.split("?")[1]}`;
    else {
      urlToLoad =
        urlToLoad.substring(0, urlToLoad.lastIndexOf("?")) +
        `/${preferences.general.startupPage}` +
        urlToLoad.substring(urlToLoad.lastIndexOf("?"));
    }
