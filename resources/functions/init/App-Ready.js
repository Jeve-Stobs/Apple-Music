...
exports.ApplicationReady = function () {
  ...
  if (app.preferences.value("window.appStartupBehavior").includes("hidden")) {
    ...
    app.preferences.value("window.appStartupBehavior").includes("minimized")
  ) {
    app.setLoginItemSettings({
      openAtLogin: true,
      args: ["--process-start-args", `"--minimized"`],
    });
  } else if (
    app.preferences.value("window.appStartupBehavior").includes("true")
  ) {
    app.setLoginItemSettings({
      openAtLogin: true,
      args: [],
