...
exports.InitializeTheme = function () {
	...
	console.log(
		`[InitializeTheme][existsSync] Checking if themes directory exists. ('${app.userThemesPath}')`
	);
	...
	if (!fs.existsSync(app.userThemesPath)) {
		...
		console.log(
			`[InitializeTheme][existsSync] Initial Themes have been copied to '${app.userThemesPath}'`
		);
	} else {
	...
	fs.access(
		join(app.userThemesPath, 'Template.css'),
		fs.constants.F_OK,
		(err) => {
			if (err) {
				console.error(
					`[InitializeTheme][access] ${err} - Template.css could not be read. Attempting to change permissions.`
				);
				chmodr(app.userThemesPath, 0o777, (err) => {
					if (err) {
						console.error(
							`[InitializeTheme][chmodr] ${err} - Theme set to default to prevent application launch halt.`
						);
						...
						console.log(
							'[InitializeTheme] Folder permissions successfully set.'
						);
					}
				});
			} else {
				...
				if (
					!app.preferences.value('advanced.themeDevelopment').includes(true)
				) {
					copySync(app.themesPath, app.userThemesPath, { overwrite: true });
					console.log(
						`[InitializeTheme][copyThemes] Themes overwritten at '${app.userThemesPath}'`
					);
				}
			}
		}
