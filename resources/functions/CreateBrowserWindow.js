...
const BrowserWindowCreation = {
	fetchTransparencyColor: function (fileName) {
		...
		if (hex_codes.length === 1) {
			return `#${hex_codes[0]}`;
		} else if (hex_codes.length === 2) {
			// This is a shitty way of doing things but I'm not gonna search for it
	...
	fetchAcrylicTheme: function () {
		...
		if (
			...
			acrylicTheme = BrowserWindowCreation.fetchTransparencyColor(
				app.preferences.value('visual.theme')
			);
		} else if (app.preferences.value('visual.theme') === 'default') {
			acrylicTheme = nativeTheme.shouldUseDarkColors
				? '#0f0f0f10'
				: '#ffffff10';
		}

		if (!acrylicTheme) {
			// If no transparency color can be found in the theme file or the theme isn't default
	...
	fetchTransparencyOptions: function () {
		...
		if (
			app.preferences.value('visual.transparencyEffect') &&
			app.preferences.value('visual.transparencyEffect') !== 'disabled' &&
			process.platform !== 'linux'
		) {
			// If a Custom Theme is being used
			if (
				app.preferences.value('visual.transparencyTheme') === 'appearance-based'
			) {
			...
			if (BrowserWindowCreation.isVibrancySupported()) {
				if (app.preferences.value('visual.transparencyEffect') === 'acrylic') {
					transparencyOptions.disableOnBlur = !!app.preferences
						.value('visual.transparencyDisableBlur')
						.includes(true);
				}
			} else {
				if (app.preferences.value('visual.transparencyEffect') === 'acrylic') {
			...
			if (app.preferences.value('visual.transparencyMaximumRefreshRate')) {
				...
				transparencyOptions.maximumRefreshRate = app.preferences.value(
					'visual.transparencyMaximumRefreshRate'
				);
			}
	...
	CreateBrowserWindow: function () {
		...
		if (
			app.preferences.value('visual.emulateMacOS').includes('left') ||
			app.preferences.value('visual.emulateMacOS').includes('right') ||
			app.preferences.value('advanced.forceDisableWindowFrame').includes(true)
		) {
			Frame = false;
			minWide = app.preferences.value('visual.streamerMode').includes(true)
				? 400
				: 300;
			minHigh = app.preferences.value('visual.streamerMode').includes(true)
				? 55
				: 300;
		} else {
			Frame = true;
		...
		const transparencyOptions =
			BrowserWindowCreation.fetchTransparencyOptions();

		// BrowserWindow Creation
		if (app.transparency) {
			acrylicWindow = require('electron-acrylic-window');
			console.log(
				'[CreateBrowserWindow] Creating BrowserWindow with transparency. Transparency Options:'
			);
		...
		if (!app.preferences.value('advanced.menuBarVisible').includes(true))
			win.setMenuBarVisibility(false); // Hide that nasty menu bar
		if (app.preferences.value('advanced.devTools') !== 'built-in')
			win.setMenu(null); // Disables DevTools
		if (app.preferences.value('advanced.devTools') === 'detached')
			win.webContents.openDevTools({ mode: 'detach' }); // Enables Detached DevTools
