...
module.exports = {
	LoadFiles: async function () {
		...
		const urlBase = app.preferences.value('advanced.useBetaSite')
		...
		function backButtonChecks() {
			...
			backButtonBlacklist.forEach(function (item) {
				if (
					matchRuleShort(app.win.webContents.getURL(), item) ||
					app.win.webContents.getURL() === item
				) {
					returnVal = true;
				}
			});
		...
		/* Load Back Button */
		if (
			app.preferences.value('visual.backButton').includes(true) &&
			!backButtonChecks() &&
			app.win.webContents.canGoBack()
		) {
			LoadJS('backButton.js');
		} else {
			/* Remove it if user cannot go back */
			await app.win.webContents.executeJavaScript(
				`if (document.querySelector('#backButtonBar')) { document.getElementById('backButtonBar').remove() };`
			);
		}
