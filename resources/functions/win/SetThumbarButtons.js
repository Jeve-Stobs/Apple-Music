...
exports.SetThumbarButtons = function (state) {
	...
	if (!Images) {
		Images = {
			next: nativeImage
				.createFromPath(trayIconDir + `next.png`)
				.resize({ width: 32, height: 32 }),
			nextInactive: nativeImage
				.createFromPath(trayIconDir + `next-inactive.png`)
				.resize({ width: 32, height: 32 }),

			pause: nativeImage
				.createFromPath(trayIconDir + `pause.png`)
				.resize({ width: 32, height: 32 }),
			pauseInactive: nativeImage
				.createFromPath(trayIconDir + `pause-inactive.png`)
				.resize({ width: 32, height: 32 }),

			play: nativeImage
				.createFromPath(trayIconDir + `play.png`)
				.resize({ width: 32, height: 32 }),
			playInactive: nativeImage
				.createFromPath(trayIconDir + `play-inactive.png`)
				.resize({ width: 32, height: 32 }),

			previous: nativeImage
				.createFromPath(trayIconDir + `previous.png`)
				.resize({ width: 32, height: 32 }),
			previousInactive: nativeImage
				.createFromPath(trayIconDir + `previous-inactive.png`)
				.resize({ width: 32, height: 32 })
		};
	}
	...
	console.log(
		app.win.setThumbarButtons(array)
			? '[setThumbarButtons] Thumbar Buttons Set.'
			: '[setThumbarButtons] Thumbar Buttons Failed to be set.'
	);
};
