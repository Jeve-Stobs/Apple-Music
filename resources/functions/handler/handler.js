...
const handler = {
	InstanceHandler: function () {
		...
		if (
			!gotTheLock &&
			!app.preferences.value('advanced.allowMultipleInstances').includes(true)
		) {
			console.log(
			...
			app.on('second-instance', (_e, argv) => {
				console.log(
					`[InstanceHandler] Second Instance Started with args: [${argv.join(
						', '
					)}]`
				);
				...
				if (argv.includes('--force-quit')) {
					...
					app.win &&
					!app.preferences
						.value('advanced.allowMultipleInstances')
						.includes(true)
				) {
	...
	PlaybackStateHandler: function () {
		...
		ipcMain.on('playbackStateDidChange', (_item, a) => {
			...
			try {
				if (a.playParams.id !== app.PreviousSongId) {
					// If it is a new song
					a.startTime = Date.now();
					a.endTime = Number(Math.round(a.startTime + a.durationInMillis));
				} else {
					// If its continuing from the same song
					a.startTime = Date.now();
					a.endTime = Number(Math.round(Date.now() + a.remainingTime));
				}
			} catch (err) {
				console.error(
					`[playbackStateDidChange] Error when setting endTime - ${err}`
				);
				a.endTime = 0;
			}
			...
			if (app.preferences.value('general.incognitoMode').includes(true)) {
				console.log(
					'[Incognito] Incognito Mode enabled. DiscordRPC and LastFM updates are ignored.'
				);
			} else {
	...
	WindowStateHandler: function () {
		...
		app.win.webContents.setWindowOpenHandler(({ url }) => {
			shell
				...
				.then(() =>
					console.log(
						`[WindowStateHandler] User has opened ${url} which has been redirected to browser.`
					)
				);
			return { action: 'deny' };
		});
		...
		app.win.webContents.on('unresponsive', async () => {
			...
			if (response === 0) {
				...
				console.log(
					'[WindowStateHandler] Application has become unresponsive and has been closed.'
				);
				app.exit();
			}
		});
		...
		app.win.webContents.on('page-title-updated', function (event) {
			// Prevents the Window Title from being Updated
			LoadFiles();
			event.preventDefault();
		});

		app.win.on('close', function (event) {
