...
exports.CreateNotification = function (attributes) {
  if (
    ...
    !(
      app.preferences.value("general.playbackNotifications").includes(true) ||
      app.preferences
        .value("general.playbackNotifications")
        .includes("minimized")
    )
  )
    return;
  ...
  if (
    app.preferences
      .value("general.playbackNotifications")
      .includes("minimized") &&
    !(!app.win.isVisible() || app.win.isMinimized())
  ) {
    return;
  }
  ...
  if (app.preferences.value("advanced.verboseLogging").includes(true)) {
    console.log(
      `[CreateNotification] Notification Generating | Function Parameters: SongName: ${attributes.name} | Artist: ${attributes.artistName} | Album: ${attributes.albumName}`
    );
  }

  if (app.ipc.existingNotification) {
    console.log(
      "[CreateNotification] Existing Notification Found - Removing. "
    );
  ...
  if (process.platform === "darwin") {
    app.ipc.existingNotification.addListener("action", (_event) => {
      app.win.webContents
        ...
        .then(() => console.log("[CreateNotification] skipToNextItem"));
    });
  }
};
